...
<template>
    <div class="single-accordion" :class="[exclusive && 'is-exclusive']">
        <details
            ...
            :class="['accordion-item', item.class && 'is-' + item.class, internalOpenItems?.includes(key) && 'is-active']"
        >
            <slot name="accordion-item" :item="item" :index="key" :toggle="toggle">
                <summary class="accordion-header" tabindex="0" @keydown.space.prevent="() => toggle(key)" @click.prevent="() => toggle(key)">
