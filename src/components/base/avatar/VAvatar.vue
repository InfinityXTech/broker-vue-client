...
export type VAvatarColor = 'primary' | 'success' | 'info' | 'warning' | 'danger' | 'h-purple' | 'h-orange' | 'h-blue' | 'h-green' | 'h-red' | 'h-yellow'
...
<template>
    <div class="v-avatar" :class="[size && `is-${props.size}`, dot && 'has-dot', dotColor && `dot-${props.dotColor}`, squared && dot && 'has-dot-squared']">
        <slot name="avatar">
            <img
                v-if="props.picture"
                class="avatar"
        ...
        <slot name="badge">
            <img v-if="props.badge" class="badge" :src="props.badge" alt="" @error.once="(event) => onceImageErrored(event, '150x150')" />
        </slot>
    </div>
</template>
