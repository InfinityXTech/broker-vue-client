...
const onUpdate = () => {
    if (!props.readonly) {
        emit('update:modelValue', checkbox.value?.checked ?? !props.modelValue)
    }
    return false
}
</script>
...
<template inherit-attrs="false">
    <div :class="[props.label && 'switch-block', props.thin && props.label && 'thin-switch-block']">
        <template v-if="props.thin">
            <label :for="blockSwitchId" class="thin-switch" tabindex="0" :class="[props.color && `is-${props.color}`]" @keydown.space.prevent="onUpdate">
                <input
                    :id="blockSwitchId"
                    ref="checkbox"
