...
const resetAngle = () => {
    if (pswpElement.value) {
        angle.value = 0
        pswpElement.value.querySelectorAll('.pswp__img').forEach((i) => ((i as HTMLElement).style.transform = `rotate(${angle.value}deg)`))
    }
}
</script>
...
<template>
    <div ref="galleryElement" class="my-gallery" itemscope itemtype="http://schema.org/ImageGallery">
        <figure
            ...
            :src="item.src"
        >
            <a :href="item.src" itemprop="contentUrl" :data-size="'' + item.w + 'x' + item.h" :title="item.title" @click.prevent="onThumbnailsClick">
                <img
                    :class="[thumbnailRadius && `radius-${thumbnailRadius}`]"
                    :src="item.thumbnail"
                    :alt="item.alt"
                    itemprop="thumbnail"
                    @error.once="(event) => onceImageErrored(event, `${item.w || '100'}x${item.h || '100'}`)"
                />
            </a>
        </figure>
    </div>
    ...
    <div ref="pswpElement" class="pswp" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <div class="pswp__scroll-wrap">
          ...
          ></button> -->
                ...
                <button class="pswp__button pswp__button--arrow--left" title="Previous (arrow left)" @click="resetAngle"></button>
                <button class="pswp__button pswp__button--arrow--right" title="Next (arrow right)" @click="resetAngle"></button>
