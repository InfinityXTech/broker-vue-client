...
export default defineComponent({
    ...
    setup(props, { slots, emit }) {
        ...
        const slotContent = (slots.default?.() ?? []).reduce((result, child) => {
            if (isSymbol(child.type) && isArray(child.children)) {
        ...
        return () => {
            ...
            const hasNestedLinkActive = computed(() =>
                slotContent.reduce((result, child) => result || (child.props?.to?.name && currentRoute === child.props.to.name), false)
            )
            const parentLink = h(
                'a',
                {
            ...
            const content = hasNestedLinkActive.value || isOpen.value ? h('ul', {}, links) : undefined

            return h(
                'li',
                {
                    class: ['collapse-links has-children', (hasNestedLinkActive.value || isOpen.value) && 'active'],
                },
                [
                    collapseWrap,
                    h(
