...
import SchemeLogList from '/@src/schemes/advertisers/billing/generalLogList.json'
import SchemeBalanceLogList from '/@src/schemes/advertisers/billing/generalBalanceLogList.json'
import SchemeLogForm from '/@src/schemes/traffic_endpoints/billing/generalLogForm.json'
import SchemeCreditForm from '/@src/schemes/advertisers/billing/generalCreditForm.json'
...
//     viewWrapper.setPageTitle('Advertiser: ' + response?.data?.name + ' - Billing General')
// })
...
api.get(`/advertisers/${props.advertiserId}/billing/general/balance`).then((response: any) => {
...
const balanceLog = (function () {
                ...
                if (!is_financial) {
                    ...
                    return h('input', {
                        ...
                        onChange: async (e: any) => {
                            ...
                            await api
                                .put(`/advertisers/${props.advertiserId}/billing/general/balance/logs/update/${row._id?.$oid ?? row._id}`, {
                                    real_balance: value,
                                })
    ...
    const fetch_data = async () => {
        try {
            ...
            const { data: response } = await api.post(`/advertisers/${props.advertiserId}/billing/general/balance/logs`, {
...
const changeLog = (function () {
    ...
    return {
        ...
        async show() {
            try {
                isLoading.value = true
                const { data: response } = await api.post(`/advertisers/${props.advertiserId}/billing/general/logs`, { extended: true })
...
const negativeBalance = (function () {
    ...
    return {
        ...
        async onStore() {
            changeTo.value = false
            reactiveReplace(general, (await api.get(`/advertisers/${props.advertiserId}/billing/general/balance`)).data)
        },
    }
})()
...
const onChangeManualStatus = async (val: string) => {
    await api.put(`/advertisers/${props.advertiserId}/billing/manual_status`, { billing_manual_status: val })
    notif.success('Updated Successfully')
}
</script>

<template>
    <AdvertiserHeader :advertiser-id="props.advertiserId" title="Billing General"></AdvertiserHeader>
        ...
        <VLoader :active="isFetching" size="xl">
            ...
            <div class="block settings-negative-balance">
                ...
                <ApiForm
                    ...
                    store-method="put"
                    :store-data="`/advertisers/${props.advertiserId}/billing/general/settings/negative_balance`"
                ...
                <ApiForm
                    ...
                    store-method="put"
                    :store-data="`/advertisers/${props.advertiserId}/billing/general/settings/credit_amount`"
            ...
            <div class="block block-log" style="clear: both">
                <h2>Log</h2>
                <Datatable :columns="SchemeLogList.columns" :uri="`/advertisers/${props.advertiserId}/billing/general/logs`" :render-row="RenderLogs" />
                <br />
            ...
            <VModal :open="changeLog.isActive.value" title="History log" size="big" cancel-label="Close" actions="right" @close="changeLog.hide">
                <template #content>
                    <ApiForm
                        ...
                        store-label="Search"
                        store-method="post"
                        :store-data="`/advertisers/${props.advertiserId}/billing/general/logs`"
