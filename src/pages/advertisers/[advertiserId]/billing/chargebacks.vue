...
import SchemeList from '/@src/schemes/advertisers/billing/chargebacksList.json'
import SchemeEdit from '/@src/schemes/advertisers/billing/chargebackEdit.json'
import SchemeNew from '/@src/schemes/advertisers/billing/chargebackEdit.json'
...
//     viewWrapper.setPageTitle('Advertiser: ' + response?.data?.name + ' - Billing Chargebacks')
// )
...
provide(
    ...
    customDictionary(`/advertisers/${props.advertiserId}/billing/payment_methods/all`, (item) => {
        const label = [
...
provide(
    ...
    customDictionary(`/advertisers/${props.advertiserId}/billing/payment_requests/completed`, (item) => {
        if (item.chargeback) {
            return undefined
        }
        return {
...
const initial = async () => {
    const { data } = await api.get(`/advertisers/${props.advertiserId}/billing/payment_methods/all`)
...
<template>
    <AdvertiserHeader :advertiser-id="props.advertiserId" title="Billing Chargebacks"></AdvertiserHeader>
    <LocalToolbar>
        <CurrencyConverter />
    </LocalToolbar>
    ...
    <Datatable
        ...
        :uri="`/advertisers/${props.advertiserId}/billing/chargeback/all`"
        :custom="custom"
    ...
    <ApiForm
        ...
        :scheme="SchemeNew"
        store-method="post"
        :store-data="`/advertisers/${props.advertiserId}/billing/chargeback/create`"
    ...
    <ApiForm
        ...
        :fetch-data="`/advertisers/${props.advertiserId}/billing/chargeback/${update}`"
        :scheme="SchemeEdit"
        store-method="post"
        :store-data="`/advertisers/${props.advertiserId}/billing/chargeback/update/${update}`"
    ...
    <ApiForm
        ...
        store-method="delete"
        :store-data="`/advertisers/${props.advertiserId}/billing/chargeback/delete/${remove}`"
