...
import Scheme from '/@src/schemes/click_report/reportForm.json'
...
viewWrapper.setPageTitle('Click Report Dashboard')

const dataValue = ref({ columns: {}, items: [] })

const router = useRouter()
const userSession = useUserSession()
const perm = userSession.permissions('click_reports')
...
if (!perm.is_active()) {
    ...
    if (reports_perm.is_active()) {
        ...
        const click_quality_reports_perm = userSession.permissions('quality_reports')
        if (click_quality_reports_perm.is_active()) {
...
const onUpdateData = (data: any, target: any, v: any) => {
    switch (target) {
        case 'pivot': {
            api.put('/auth/user_profile_data/click_report_pivot', { data: v })
            break
        }
        case 'metrics': {
            api.put('/auth/user_profile_data/click_report_metrics', { data: v })
            break
        }
    }
}
...
const exports = (function () {
    ...
    return {
        ...
        download() {
            ...
            download.post('/click_report/download', query, 'click_report-report' + datetime + '.csv')
        },
    }
})()
</script>
...
<template>
    ...
    <Datatable :columns="dataValue.columns" :custom="custom" :model-value="dataValue.items" :show-only-screen-data="true" />
    <VModal
