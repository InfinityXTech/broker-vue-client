...
import SchemeList from '/@src/schemes/affiliates/billing/paymentMethodsList.json'
import SchemeNew from '/@src/schemes/affiliates/billing/paymentMethodsNew.json'
...
api.get('/affiliates/' + props.affiliateId).then((response: any) =>
    viewWrapper.setPageTitle('Affiliate: #' + response?.data?.token + ' - Billing Payment Methods')
)
    ...
    Details1() {
        return (value: any, row: any) => {
            ...
            if (row.payment_method == 'crypto') {
                ...
                return h('div', { innerHTML: 'Wallet: ' + row.wallet })
            }
            return h('div', { innerHTML: row.payment_method })
        }
    },
    Details2() {
        return (value: any, row: any) => {
...
<template>
    ...
    <ApiForm
        ...
        store-method="post"
        :store-data="`/affiliates/${props.affiliateId}/billing/payment_methods/create`"
    ...
    <Datatable
        ...
        :uri="`/affiliates/${props.affiliateId}/billing/payment_methods/all`"
        :custom="custom"
    ...
    <ApiForm
        ...
        store-method="put"
        :store-data="`/affiliates/${props.affiliateId}/billing/payment_methods/select/${select}`"
