...
import SchemeList from '/@src/schemes/traffic_endpoints/sub_publisher_tokensList.json'
import SchemeNew from '/@src/schemes/traffic_endpoints/sub_publisher_tokensNew.json'
...
const is_access_edit = true //perm.is_allow('all', 'edit') && perm.is_allow('security')
const is_access_add = true //perm.is_allow('all', 'add') && perm.is_allow('security')
            ...
            if (is_access_edit) {
                return h(
                    ApiCallButton,
                    {
                        ...
                        modelValue: value,
                        method: 'DELETE',
                        uri: '/traffic_endpoint/' + props.id + '/sub_publisher_tokens/delete/' + proxy._id,
...
<template>
    ...
    <div class="traffic-endpoint-sub_publisher_tokens">
        <div class="list-flex-toolbar">
            <VButtons>
                <VButton v-if="is_access_add" color="primary" icon="fas fa-plus" elevated @click="create = true"> Generate Sub publisher token </VButton>
            </VButtons>
        </div>
        ...
        <Datatable
            ...
            :uri="`/traffic_endpoint/${props.id}/sub_publisher_tokens/all`"
            :custom="{}"
        ...
        <ApiForm
            v-if="create"
            :wrapper="VModal"
            title="Generate Sub publisher token"
            :fetch-data="initial"
            :scheme="SchemeNew"
            store-method="post"
            :store-data="`/traffic_endpoint/${props.id}/sub_publisher_tokens/create`"
            @store-data="onStored"
            @close="create = false"
        />
    </div>
...
.traffic-endpoint-sub_publisher_tokens {
