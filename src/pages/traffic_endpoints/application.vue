...
import SchemeSearch from '/@src/schemes/traffic_endpoints/endpointsApplicationSearch.json'
import TrafficEndpointTableScheme from '/@src/schemes/traffic_endpoints/endpointsApplicationList.json'
...
const custom = {
    ...
    ActionsIcons() {
        return (value: string, row: any) => {
            ...
            //Approve
            if (row.UnderReview == 0 || row.UnderReview == 2) {
                actions.push(
                    h(
                        VButton,
                        {
            ...
            //Reject
            if (row.UnderReview == 0 || row.UnderReview == 1) {
                if (actions.length > 0) {
                    actions.push(' ')
                }
...
const formData = { UnderReview: '0' } //'0,1,2'
...
<template>
    ...
    <ApiForm
        v-if="approve"
        :wrapper="VModal"
        title="Approve TrafficEndpoint Application"
        :fetch-data="{}"
        :scheme="schemeConfirm('Are you sure you want to approve this record?')"
        store-method="put"
        :store-data="`/traffic_endpoint/${approve}/application/approve`"
        @store-data="onStored"
        @close="approve = false"
    />
    ...
    <ApiForm
        v-if="reject"
        :wrapper="VModal"
        title="Reject TrafficEndpoint Application"
        :fetch-data="{}"
        :scheme="schemeConfirm('Are you sure you want to reject this record?')"
        store-method="put"
        :store-data="`/traffic_endpoint/${reject}/application/reject`"
        @store-data="onStored"
        @close="reject = false"
    />
</template>
