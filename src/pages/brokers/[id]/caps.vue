...
import { h, provide, ref, onUnmounted, computed } from 'vue'
...
let is_access_edit = perm.is_allow('all', 'edit') && perm.is_allow('daily_cap')
let is_access_add = perm.is_allow('all', 'add') && perm.is_allow('daily_cap')
...
const storeCap = (data: any) => {
    caps.value = data
    datatable.value?.forceReRenderForm(null)
}
