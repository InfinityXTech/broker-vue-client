...
const custom = {
    BrokerName() {
        return (value: string, row: any) => {
            return [h(RouterLink, { to: { name: 'brokers-id', params: { id: row._id } } }, () => h('b', value)), h('br'), h('small', row._id)]
        }
    },
    AccountName() {
        return (value: any) => {
...
<template>
    <LocalToolbar>
        <ToolbarItem
            ...
            @click="download.get('/broker/crgdeals/download')"
        />
        <ToolbarItem v-if="is_access_download_price" icon="fas fa-download" title="Download Price" @click="download.get('/broker/price/download')" />
    </LocalToolbar>
