...
import SchemeList from '/@src/schemes/brokers/billing/paymentMethodsList.json'
...
api.get('/broker/' + props.id).then((response: any) =>
    viewWrapper.setPageTitle('Broker: ' + response?.data?.partner_name_secure + ' - Billing Payment Methods')
)
